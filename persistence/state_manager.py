...
class StateManager:
    ...
    def __init__(self, order_manager, portfolio_manager, settings, state_file='data/bot_state.json', logger=None):
        ...
        self.backup_manager = BackupManager(state_file_path=state_file, max_backups=10, logger=self.logger)
        ...
        try:
            ...
            self.error_logger.critical(
                f'Failed to create directory for state file {self.state_file}: {e}. State saving/loading will fail.'
            )
        ...
        with self._lock:  # Acquire lock for the entire save operation
            try:
                ...
                self.error_logger.error(
                    f"Failed to save state: Component missing 'get_state' method? {ae}", exc_info=True
                )
                return False
        ...
        with self._lock:  # Acquire lock for the entire load operation
            ...
            try:
                ...
                if 'order_manager' in state:
                    ...
                    self.logger.warning("No 'order_manager' state found in file.")
                ...
                if 'portfolio_manager' in state:
                    ...
                    self.logger.warning("No 'portfolio_manager' state found in file.")
                ...
                if load_success:
                    ...
                    self.logger.info(
                        f'Bot state successfully loaded from {self.state_file} (Saved At: {load_time_str})'
                    )
                    return True  # Indicate state was loaded successfully
                else:
                    self.error_logger.error(
                        f'Partial or complete failure loading state from {self.state_file}. Bot may start with incorrect state.'
                    )
                ...
                self.error_logger.critical(
                    f'Failed to decode JSON from state file {self.state_file}: {jde}. Starting fresh.', exc_info=True
                )
                return False
            except Exception as e:
                self.error_logger.critical(
                    f'Failed to load bot state from {self.state_file}: {e}. Starting fresh.', exc_info=True
                )
                return False  # Indicate state was not loaded
