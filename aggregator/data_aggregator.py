...
class DataAggregator:
            ...
            try:
                ...
                if not isinstance(timestamp_utc, datetime) or timestamp_utc.tzinfo is None:
                    self.error_logger.warning(
                        f'Aggregator received tick for {symbol} with invalid/naive timestamp: {timestamp_utc}. Using current UTC time.'
                    )
                ...
                if callable(self.error_callback):
                    try:
                        self.error_callback(f'Aggregator error: {e}')
                    except Exception as cb_e:
                        self.error_logger.error(f'Error calling aggregator error callback: {cb_e}', exc_info=True)
    ...
    def _process_tick(self, symbol, tick_type, price, size, timestamp_utc):
        ...
        ohlc_price = None
        if tick_type == 4:
            ohlc_price = price  # Use LAST price for OHLC
        ...
        with self._subscribe_lock:
            ...
            if last_processed_time is not None and timestamp_utc < last_processed_time:
                self.logger.warning(
                    f'Ignoring out-of-order tick for {symbol}: Tick time {timestamp_utc} < Last processed {last_processed_time}'
                )
                return
            ...
            if q:
                if q.full():
                    ...
                    try:
                        ...
                        pass  # Ignore if became empty concurrently
                q.put(bar_df)  # Put the new bar DataFrame
                ts_str = finished_bar['timestamp'].strftime(self.settings.LOG_TIMESTAMP_FORMAT)
                self.logger.info(
                    f'Finalized UTC bar for {symbol}: {ts_str} O={finished_bar["open"]:.5f} H={finished_bar["high"]:.5f} L={finished_bar["low"]:.5f} C={finished_bar["close"]:.5f} V={finished_bar["volume"]:.2f}'
                )
            else:
    ...
    def stop(self):
        ...
        if self.thread and self.thread.is_alive():
            ...
            if self.thread.is_alive():
                self.logger.warning('Data Aggregator thread did not join cleanly.')
            else:
                self.logger.info('Data Aggregator thread joined.')
        self.thread = None
