...
class ConfigValidator:
    ...
    def _validate_risk_parameters(settings) -> list[ValidationError]:
        ...
        if max_daily_loss is None:
            errors.append(
                ValidationError(
                    'MAX_DAILY_LOSS is not set', 'Add MAX_DAILY_LOSS to config/settings.py (recommended: 0.03 for 3%)'
                )
            )
    ...
    def _validate_instruments(settings) -> list[ValidationError]:
        ...
        for instrument in instruments:
            ...
            elif trading_mode == 'forex' and not ConfigValidator.FOREX_PATTERN.match(instrument):
                errors.append(
                    ValidationError(
...
class InputValidator:
    ...
    def validate_number_input(
        value: str, min_val: Optional[float] = None, max_val: Optional[float] = None, allow_negative: bool = False
