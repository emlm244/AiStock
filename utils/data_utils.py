...
def calculate_position_size(
    ...
    if entry_price is None or stop_loss_price is None or risk_amount is None or entry_price <= 0:
        logger.error(f"Invalid input for position size calculation: Entry={entry_price}, SL={stop_loss_price}, RiskAmt={risk_amount}")
        return 0.0
    if min_trade_size <= 0 or min_tick <= 0:
         logger.error(f"Invalid contract details for position size calculation: MinSize={min_trade_size}, MinTick={min_tick}")
         return 0.0
    ...
    if raw_position_size < min_trade_size:
        ...
        logger.warning(f"Calculated size {raw_position_size:.8f} for {symbol} is below minimum increment {min_trade_size:.8f}. Cannot meet risk/size constraints.")
    ...
    if position_size < min_trade_size: # Double check after rounding
         logger.warning(f"Position size {position_size:.8f} for {symbol} fell below minimum {min_trade_size:.8f} after rounding. Cannot place trade.")
         return 0.0
