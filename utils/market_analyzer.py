...
try:
    ...
    from indicators.volatility import calculate_atr, calculate_bollinger_bands_width
...
class MarketRegimeDetector:
    ...
    def detect_regime(self, symbol, market_data_df):
        ...
        if not isinstance(market_data_df.index, pd.DatetimeIndex) or market_data_df.index.tz is None:
            self.error_logger.warning(
                f'Regime detection for {symbol}: Data has invalid index. Requires UTC DatetimeIndex.'
            )
            self.current_regimes[symbol] = default_regime
            return default_regime.copy()
        ...
        if not all(col in market_data_df.columns for col in required_cols):
            self.error_logger.warning(
                f'Regime detection for {symbol}: Data missing required columns (high, low, close).'
            )
            self.current_regimes[symbol] = default_regime
            return default_regime.copy()
        ...
        try:
            ...
            indicators_valid = all(
                pd.notna(val)
                for val in [latest_adx, latest_plus_di, latest_minus_di, latest_bbw, latest_atr, latest_close]
            )
            if not indicators_valid or latest_close <= 0 or latest_atr < self.min_atr_value:
                self.logger.warning(
                    f'Regime calc warning for {symbol}: NaN or invalid indicator/price value found. '
            ...
            if previous_regime != combined_regime:
                self.logger.info(
                    f'Regime change for {symbol}: {previous_regime} -> {combined_regime} (Indicators: {indicator_values})'
                )
            else:
    ...
    def get_regime(self, symbol):
        ...
        return self.current_regimes.get(
            symbol,
            {
                'trend': 'Unknown',
                'volatility': 'Unknown',
