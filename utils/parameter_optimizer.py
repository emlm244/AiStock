...
class AdaptiveParameterOptimizer:
    ...
    def __init__(self, settings, trade_history_getter, regime_detector, logger=None):
        ...
        if self.settings.STOP_LOSS_TYPE != 'ATR':
            self.parameter_bounds.pop('STOP_LOSS_ATR_MULTIPLIER', None)
        if self.settings.TAKE_PROFIT_TYPE != 'ATR':
            self.parameter_bounds.pop('TAKE_PROFIT_ATR_MULTIPLIER', None)
        if self.settings.TAKE_PROFIT_TYPE != 'RATIO':
            self.parameter_bounds.pop('TAKE_PROFIT_RR_RATIO', None)
        ...
        self.strategy_performance_snapshot = defaultdict(
            lambda: defaultdict(lambda: {'trades': 0, 'wins': 0, 'win_rate': 0.0})
        )
    ...
    def _get_setting(self, setting_path, default=None):
        ...
        try:
            ...
            for key in keys:
                ...
                if value is None:
                    return default  # Stop if intermediate key not found
            return value
        ...
        try:
            ...
            for key in keys[:-1]:
                if isinstance(obj, dict):
                    if key not in obj:
                        obj[key] = {}  # Create dict if needed for path
                    obj = obj.get(key)
                else:
                    ...
                    if not hasattr(obj, key):
                        self.error_logger.error(
                            f"Optimizer: Cannot navigate setting path '{setting_path}'. Missing intermediate attribute '{key}'."
                        )
                        return False
                    obj = getattr(obj, key)
                ...
                if not isinstance(obj, (dict, object)):  # Check if parent is valid container
                    self.error_logger.error(
                        f"Optimizer: Invalid intermediate object type for path '{setting_path}' at key '{key}'. Type: {type(obj)}"
                    )
                    return False
            ...
            if current_value is not None and not isinstance(new_value, type(current_value)):
                try:
                    ...
                    self.error_logger.warning(
                        f"Optimizer: Type mismatch for '{setting_path}'. Cannot convert {new_value} to {type(current_value)}. Error: {e}"
                    )
            ...
            if changed:
                if isinstance(obj, dict):
                    ...
                    self.error_logger.error(
                        f"Optimizer: Cannot set setting '{setting_path}'. Parent is not dict and has no attribute '{final_key}'. Parent type: {type(obj)}"
                    )
                    return False
            ...
            self.error_logger.error(
                f"Optimizer: Failed to set setting '{setting_path}' to {new_value}: {e}", exc_info=True
            )
            return False
    ...
    def _clamp(self, value, min_val, max_val):
        ...
        if value is None:
            return min_val  # Or some other default?
    ...
    def update_parameters(self, force=False):
        ...
        self.logger.info(
            f'--- Running Parameter Optimization Heuristics ({current_time_utc.strftime(self.settings.LOG_TIMESTAMP_FORMAT)}) ---'
        )
        ...
        for symbol_data in self.strategy_performance_snapshot.values():
            for _strat_name, strat_data in symbol_data.items():
        ...
        self.logger.info(
            f'Optimizer: Analyzing trades from last {self.lookback_period} across {len(symbols_to_optimize)} symbols for potential adaptation.'
        )
        ...
        for setting_path, bounds in self.parameter_bounds.items():
            ...
            if not strategy_key:
                continue  # Skip if parameter doesn't map to a known strategy type
            ...
            for symbol in symbols_to_optimize:
                ...
                if (
                    symbol in self.strategy_performance_snapshot
                    and strategy_key in self.strategy_performance_snapshot[symbol]
                ):
                    ...
                    if perf_data['trades'] >= self.min_trades_for_eval:
                        ...
                        self.logger.debug(
                            f"Optimizing '{setting_path}' for {symbol} (WinRate: {perf_data['win_rate']:.1%}, Trades: {perf_data['trades']}, Trend: {trend_regime}, Vol: {vol_regime})"
                        )
        ...
        self.logger.info('--- Parameter Optimization Heuristics Finished ---')
    ...
    def _get_strategy_key_from_setting(self, setting_path):
        ...
        if 'moving_average' in path_lower:
            return 'TrendFollowingStrategy'
        if 'rsi' in path_lower:
            return 'MeanReversionStrategy'
        if 'momentum' in path_lower:
            return 'MomentumStrategy'
        if 'macd' in path_lower:
            return 'MLStrategy'  # Or common indicator pool? Assume ML features for now.
        if 'stop_loss' in path_lower or 'take_profit' in path_lower:
            return 'Risk'  # Special category for risk params
        return None
    ...
    def _optimize_parameter(self, setting_path, bounds, win_rate, trend_regime, vol_regime):
        ...
        # Mean Reversion (RSI Period, Levels)
            ...
            if 'RSI_PERIOD' in setting_path:
                ...
                if 'Ranging' in trend_regime:  # Ranging Market -> favor MR
                    if win_rate < target_win_rate - 0.05:
                        adjustment_direction = -1  # Shorten period if losing (missing reversals?)
                else:  # Trending Market -> penalize MR
                    if win_rate < target_win_rate - 0.10:
                        adjustment_direction = 1  # Lengthen period if losing (fighting trend)
            elif 'RSI_OVERBOUGHT' in setting_path:  # OB Threshold
                adjustment_magnitude = 1
                if win_rate < target_win_rate:
                    adjustment_direction = -1  # Lower threshold if losing (selling too early?)
            elif 'RSI_OVERSOLD' in setting_path:  # OS Threshold
                adjustment_magnitude = 1
                if win_rate < target_win_rate:
                    adjustment_direction = 1  # Raise threshold if losing (buying too early?)
        ...
        # Momentum
            ...
            if 'PERIOD' in setting_path:
                ...
                if 'Trending' in trend_regime or vol_regime == 'High':  # Momentum might work here
                    if win_rate < target_win_rate - 0.05:
                        adjustment_direction = -1  # Faster lookback if missing moves
                else:  # Ranging / Low Vol / Squeeze -> penalize Momentum
                    if win_rate < target_win_rate - 0.10:
                        adjustment_direction = 1  # Slower lookback if false signals
                ...
                if vol_regime == 'High':
                    if win_rate < target_win_rate:
                        adjustment_direction = 1  # Increase threshold if too sensitive
                else:  # Lower Vol
                    if win_rate < target_win_rate:
                        adjustment_direction = -1  # Decrease threshold if too insensitive
        ...
        # Risk Parameters (Base Multipliers/Ratio)
            ...
            if setting_path == 'TAKE_PROFIT_RR_RATIO':
                adjustment_magnitude = 0.1  # Slightly larger for ratio

            # Simple logic: If win rate is poor (<40%), tighten risk slightly. If good (>60%), loosen slightly.
            if win_rate < 0.40:
                if 'STOP_LOSS' in setting_path:
                    adjustment_direction = -1  # Tighten SL slightly (e.g., reduce ATR mult)
                if 'TAKE_PROFIT' in setting_path:
                    adjustment_direction = -1  # Reduce TP target slightly
            elif win_rate > 0.60:
                if 'STOP_LOSS' in setting_path:
                    adjustment_direction = 1  # Loosen SL slightly (controversial)
                if 'TAKE_PROFIT' in setting_path:
                    adjustment_direction = 1  # Increase TP target slightly
        ...
        if adjustment_direction != 0:
            ...
            if not is_float:  # Integer parameter
                ...
                if '.' in str(base_step):
                    precision = len(str(base_step).split('.')[-1])
            ...
            if setting_path == 'MOVING_AVERAGE_PERIODS.short_term':
                ...
                if long_val is not None:
                    final_value = min(final_value, long_val - 1)
