...
def test_audit_logger_hash_chain(tmp_path: Path) -> None:
    config = AuditConfig(
...
def test_state_store_writes_payload(tmp_path: Path) -> None:
...
def test_compliance_reporter_summary(tmp_path: Path) -> None:
    config = AuditConfig(
        log_path=str(tmp_path / 'audit.jsonl'),
        state_root=str(tmp_path / 'state'),
    )
