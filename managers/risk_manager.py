...
class RiskManager:
    ...
    def _check_reset_daily_limits(self):
        ...
        if today_local > self._last_daily_reset_date:
            self.logger.info(
                f'RiskManager: New day ({today_local} in {self.default_tz.zone}). Resetting daily loss tracking.'
            )
    ...
    def check_portfolio_risk(self, latest_prices):
        ...
        if self._trading_halted and self._halted_by_drawdown:
            ...
            if current_drawdown < recovery_threshold:
                self.logger.warning(
                    f'Drawdown recovered below threshold ({current_drawdown:.2%} < {recovery_threshold:.2%}). Resuming trading.'
                )
                self._trading_halted = False
                ...
                self.logger.warning(
                    f'Trading remains halted due to Max Drawdown ({current_drawdown:.2%} >= limit {self.settings.MAX_DRAWDOWN_LIMIT:.2%}). Recovery needed below {recovery_threshold:.2%}.'
                )
                return
        ...
        if (
            current_drawdown > 0
            and not np.isclose(current_drawdown, max_drawdown_limit)
            and current_drawdown > max_drawdown_limit
        ):
            self._trading_halted = True
    ...
    def check_pre_trade_risk(self, symbol, action, quantity, price, available_funds):
        ...
        if max_single_pos_pct is not None and max_single_pos_pct > 0:
            ...
            if potential_value > max_allowed_value:
                self.logger.warning(
                    f'Pre-trade check failed for {symbol}: Potential position value ({potential_value:.2f}) would exceed limit ({max_allowed_value:.2f} = {max_single_pos_pct:.1%} of equity).'
                )
                return False
