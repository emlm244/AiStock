...
from utils.market_analyzer import MarketRegimeDetector # Import Regime Detector
# Import strategy classes
from strategies import TrendFollowingStrategy, MeanReversionStrategy, MomentumStrategy, MLStrategy
...
class StrategyManager:
    ...
    def __init__(self, settings, portfolio_manager, regime_detector, logger=None):
        ...
        self.strategy_performance = defaultdict(lambda: {name: self._default_perf_metrics() for name in self.strategy_names})
        # Weights {symbol: {strat_name: weight}}
        self.strategy_weights = defaultdict(lambda: {name: 1.0 for name in self.strategy_names})
        # --- End State ---
        ...
        self.perf_update_interval = timedelta(minutes=getattr(settings, 'STRAT_PERF_UPDATE_INTERVAL_MIN', 30)) # Configurable interval
        ...
        self.regime_base_weights = {
            ...
            'Trending Up': {'TrendFollowingStrategy': 1.5, 'MeanReversionStrategy': 0.5, 'MomentumStrategy': 1.2, 'MLStrategy': 1.0},
            'Trending Down': {'TrendFollowingStrategy': 1.5, 'MeanReversionStrategy': 0.5, 'MomentumStrategy': 1.2, 'MLStrategy': 1.0},
            'Ranging': {'TrendFollowingStrategy': 0.5, 'MeanReversionStrategy': 1.5, 'MomentumStrategy': 0.8, 'MLStrategy': 1.0},
            # Volatility Regime adjusts overall sensitivity / confidence
            'Low': {'TrendFollowingStrategy': 0.8, 'MeanReversionStrategy': 1.2, 'MomentumStrategy': 0.7, 'MLStrategy': 0.9},
            'Normal': {'TrendFollowingStrategy': 1.0, 'MeanReversionStrategy': 1.0, 'MomentumStrategy': 1.0, 'MLStrategy': 1.0},
            'High': {'TrendFollowingStrategy': 1.2, 'MeanReversionStrategy': 0.8, 'MomentumStrategy': 1.2, 'MLStrategy': 1.1},
            'Squeeze': {'TrendFollowingStrategy': 0.7, 'MeanReversionStrategy': 1.1, 'MomentumStrategy': 0.5, 'MLStrategy': 0.8},
            # Default if regime is Unknown
            'Default': {name: 1.0 for name in self.strategy_names}
        }
    ...
    def _load_strategies(self):
        ...
        try:
            ...
            if enabled.get('trend_following', False): strategies.append(TrendFollowingStrategy())
            if enabled.get('mean_reversion', False): strategies.append(MeanReversionStrategy())
            if enabled.get('momentum', False): strategies.append(MomentumStrategy())
                 ...
                 try:
                     ...
                     if ml_strategy.is_model_loaded:
                         strategies.append(ml_strategy)
                          ...
                          self.error_logger.error("ML Strategy enabled but model failed to load during initialization. Strategy disabled.")
        ...
        for s in self.strategies:
            try:
                ...
                if isinstance(required, (int, float)) and required > 0:
                     ...
                     self.error_logger.warning(f"{s.__class__.__name__} returned invalid min_data_points: {required}. Using fallback 200.")
    ...
    def update_performance_and_weights(self, current_time_utc):
        ...
        new_strategy_performance = defaultdict(lambda: {name: self._default_perf_metrics() for name in self.strategy_names})
        ...
        risk_free_rate_annual = 0.0 # Assume 0 risk-free rate for simplicity
        periods_per_year = 252 * (24*60*60 / self.settings.TIMEFRAME_SECONDS) if hasattr(self.settings, 'TIMEFRAME_SECONDS') else 252 # Estimate periods/year
        ...
        new_strategy_weights = defaultdict(lambda: {name: 1.0 for name in self.strategy_names})
        ...
        for symbol in symbols_traded:
            ...
            perf_modifiers = {name: 1.0 for name in self.strategy_names} # Default modifier
            if valid_sharpes_exist and len(symbol_sharpes) > 1:
                 ...
                 if max_s > min_s: # Avoid division by zero if all sharpes are equal
                     ...
                     for name in symbol_sharpes: perf_modifiers[name] = 1.2 # Give slight boost
                 elif max_s <= 0: # All valid sharpes are equal and non-positive
                     for name in symbol_sharpes: perf_modifiers[name] = 0.8 # Give slight penalty
            ...
            self.logger.info(f"Updated Weights for {symbol} (Regime: {regime_info.get('regime', 'N/A')}): {new_strategy_weights[symbol]}")
            ...
            try:
                ...
                if len(data_df) >= min_pts:
                    ...
                    signal = strategy.generate_signal(data_df.copy())
                     ...
                     self.logger.debug(f"StrategyManager: Insufficient data for {name} on {symbol} ({len(data_df)}/{min_pts})")
            except Exception as e:
