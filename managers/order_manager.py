...
from api.ibkr_api import ORDER_ACTIVE_STATES, ORDER_FINAL_STATES
...
class OrderManager:
            ...
            for order_id, data in self.active_orders.items():
                ...
                serializable_orders[order_id] = {
                    'order_details': {
                        ...
                        'lmtPrice': data['order'].lmtPrice,
                        'auxPrice': data['order'].auxPrice,
                        'transmit': data['order'].transmit,
                        'parentId': data['order'].parentId,
                        'tif': data['order'].tif,
                    },
                    'contract_details': {
                        'symbol': data['contract'].symbol,
                        'secType': data['contract'].secType,
    ...
    def reconcile_orders(self, broker_open_orders):
        ...
        with self._lock:
            self.logger.info(
                f'Reconciling orders. Local active: {len(self.active_orders)}, Broker open: {len(broker_open_orders)}'
            )
            # Use the imported set ORDER_FINAL_STATES
            local_active_ids = {
                oid for oid, data in self.active_orders.items() if data['status'] not in ORDER_FINAL_STATES
            }
            ...
            for order_id in potentially_inactive_ids:
                ...
                self.logger.warning(
                    f'Order {order_id} (Status: {old_status}) active locally but not reported open by broker. Marked Inactive.'
                )
            ...
            for order_id in missing_locally_ids:
                ...
                if (
                    local_entry is None
                    or local_entry['status'] in ORDER_FINAL_STATES
                    or local_entry['status'] == 'Inactive'
                ):
                    self.logger.warning(
                        f'Order {order_id} reported open by broker but missing or inactive locally. Adding/Updating.'
                    )
            ...
            for order_id in matching_ids:
                ...
                if self.active_orders[order_id]['status'] != broker_status:
                    self.logger.info(
                        f'Updating status for order {order_id}: {self.active_orders[order_id]["status"]} -> {broker_status}'
                    )
    ...
    def create_and_place_bracket_order(
        self, symbol, action, quantity, entry_price, stop_loss_price, take_profit_price, strategy_name
    ):
        ...
        with self._lock:
            ...
            self.active_orders[parent_order_id] = {
                ...
                'parent_id': None,
                'children_ids': [stop_order_id, profit_order_id],
                'last_update_utc': now_utc,
            }
            ...
            if success:
                ...
                for placed_id in placed_ids:
                    self.logger.info(
                        f'Attempting cancellation of potentially placed order ID {placed_id} due to bracket failure.'
                    )
        ...
        with self._lock:
            ...
            if order_id in self.active_orders:
                ...
                if order_data['status'] != status or order_data.get('filled') != filled:
                    ...
                    self.logger.info(
                        f'OrderStatus Update - ID:{order_id}, Status:{status}, Filled:{filled}, Remaining:{remaining}, AvgFillPrice:{avgFillPrice}'
                    )
    ...
    def _handle_final_status(self, order_id, status):
        ...
        if parent_id is not None and parent_id in self.bracket_orders:
            ...
            if order_id == stop_id:
                sibling_id = profit_id
            elif order_id == profit_id:
                sibling_id = stop_id
            ...
            if sibling_id:
                ...
                if sibling_data and sibling_data['status'] not in ORDER_FINAL_STATES:
                    self.logger.info(
                        f'Order {order_id} (child of {parent_id}) is final ({status}). Cancelling sibling order {sibling_id}.'
                    )
    ...
    def handle_execution(self, execution: Execution, contract: Contract, timestamp_utc: datetime):
        ...
        with self._lock:
            ...
            self.logger.info(
                f'Received execution: OrderID={order_id}, ExecID={exec_id}, Symbol={symbol}, Side={execution.side}, Qty={execution.shares}, Price={execution.price}, Time={timestamp_utc.isoformat()}'
            )
            ...
            if order_id in self.active_orders:
                ...
                self.logger.warning(
                    f'Received execution details for unknown or inactive order ID: {order_id}. Processing position update.'
                )
    ...
    def cancel_order(self, order_id):
        ...
        with self._lock:
            ...
            if order_data and order_data['status'] not in ORDER_FINAL_STATES:
                ...
                self.logger.warning(
                    f'Attempted to cancel order {order_id} already in final state: {order_data["status"]}'
                )
            else:
    ...
    def _remove_order_internal(self, order_id):
        ...
        with self._lock:
            ...
            if order_id in self.order_strategy_mapping:
                del self.order_strategy_mapping[order_id]
            if order_id in self.bracket_orders:
                del self.bracket_orders[order_id]
