...
try:
    ...
    print(
        f'CRITICAL: Failed to import core config/logging: {e}. Ensure config/settings.py and utils/logger.py exist. Exiting.'
    )
    sys.exit(1)
...
class TradingBot:
    def __init__(self):
        ...
        self.logger.info(
            f'Default Settings - Mode: {self.settings.TRADING_MODE}, '
        ...
        self.logger.info(
            f'Effective Settings - Mode: {self.settings.TRADING_MODE}, '
        ...
        try:
            ...
            if len(parts) == 1 and parts[0].isdigit():  # Assume minutes if only number given
                ...
                raise ValueError(f"Invalid TIMEFRAME format: '{self.settings.TIMEFRAME}'")

            if unit.startswith('sec'):
                bar_size_td = timedelta(seconds=value)
            elif unit.startswith('min'):
                bar_size_td = timedelta(minutes=value)
            elif unit.startswith('hour'):
                bar_size_td = timedelta(hours=value)
            elif unit.startswith('day'):
                bar_size_td = timedelta(days=value)
            else:
        ...
        if self.settings.ENABLE_ADAPTIVE_RISK and (
            'ATR' in self.settings.STOP_LOSS_TYPE or 'ATR' in self.settings.TAKE_PROFIT_TYPE
        ):
        ...
        self._retrain_executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=1
        )  # Background executor for training
    ...
    def prompt_user_config(self, args=None):
        ...
        if headless and args.mode:
            ...
            while True:
                ...
                if choice == '1':
                    ...
                    self.settings.TRADING_MODE = 'forex'
                    break
                else:
                    print('Invalid input.')
        ...
        if headless and args.instruments:
            self.settings.TRADE_INSTRUMENTS = [
                inst.strip().upper() for inst in args.instruments.split(',') if inst.strip()
            ]
        elif not headless:
            ...
            if instr_input:
                self.settings.TRADE_INSTRUMENTS = [
                    inst.strip().upper() for inst in instr_input.split(',') if inst.strip()
                ]
            else:
        ...
        if headless:
            ...
            while True:
                choice = input('Enable Autonomous Mode? (Adapts strategies/risk) [Y/n]: ').strip().lower()
                if choice in ['y', 'yes', '']:
                    self.settings.AUTONOMOUS_MODE = True
                    break
                elif choice in ['n', 'no']:
                    self.settings.AUTONOMOUS_MODE = False
                    break
                else:
                    print('Invalid input.')
        ...
        if self.settings.AUTONOMOUS_MODE:
            if headless:
                ...
                self.settings.ENABLE_DYNAMIC_STRATEGY_WEIGHTING = (
                    args.dynamic_weighting if hasattr(args, 'dynamic_weighting') else True
                )
            else:
                while True:
                    choice = input('  Enable Adaptive Risk? (Adapts SL/TP based on volatility) [Y/n]: ').strip().lower()
                    if choice in ['y', 'yes', '']:
                        self.settings.ENABLE_ADAPTIVE_RISK = True
                        break
                    elif choice in ['n', 'no']:
                        self.settings.ENABLE_ADAPTIVE_RISK = False
                        break
                    else:
                        print('Invalid input.')
                ...
                while True:
                    choice = input('  Enable Automated ML Retraining? (If ML strategy enabled) [Y/n]: ').strip().lower()
                    if choice in ['y', 'yes', '']:
                        self.settings.ENABLE_AUTO_RETRAINING = True
                        break
                    elif choice in ['n', 'no']:
                        self.settings.ENABLE_AUTO_RETRAINING = False
                        break
                    else:
                        print('Invalid input.')
                ...
                while True:
                    choice = input('  Enable Dynamic Strategy Weighting? [Y/n]: ').strip().lower()
                    if choice in ['y', 'yes', '']:
                        self.settings.ENABLE_DYNAMIC_STRATEGY_WEIGHTING = True
                        break
                    elif choice in ['n', 'no']:
                        self.settings.ENABLE_DYNAMIC_STRATEGY_WEIGHTING = False
                        break
                    else:
                        print('Invalid input.')
                self.logger.info(
                    f'Dynamic Strategy Weighting set to: {self.settings.ENABLE_DYNAMIC_STRATEGY_WEIGHTING}'
                )
        ...
        if self.settings.TRADING_MODE == 'stock':
            ...
            self.settings.CONTINUE_AFTER_CLOSE = True
            self.logger.info('Continue After Close set to True (Crypto/Forex Mode).')
    ...
    def start(self):
        ...
        if state_loaded_ok:
            ...
            if os.path.exists(self.state_manager.state_file):
                self.error_logger.critical(
                    f"Failed to load existing state file '{self.state_manager.state_file}'. Starting fresh, but review the file!"
                )
        ...
        if not self._wait_for_contract_details(timeout=30):
            self.logger.warning(
                'Contract details request may be incomplete (timeout or error). Proceeding with heuristics.'
            )
    ...
    def stop(self, reason='Unknown'):
        ...
        if self.main_thread and self.main_thread.is_alive():
            ...
            if self.main_thread.is_alive():
                self.logger.warning('Main loop did not exit cleanly after 15 seconds.')
        ...
        if self.settings.CANCEL_ORDERS_ON_EXIT:
            ...
            if hasattr(self, 'order_manager') and self.order_manager and self.api.is_connected():
                ...
                self.logger.warning('Cannot cancel orders: Order Manager unavailable.')
        else:
        ...
        if hasattr(self, 'state_manager') and self.state_manager:
            ...
            self.logger.warning('State manager not available, cannot save final state.')
    ...
    def request_historical_data(self, symbol):
        ...
        with self._lock:  # Protect access to buffer/req_ids
            if symbol in self.historical_data_req_ids:  # Check tracking dictionary
                self.logger.warning(
                    f'Historical data request already pending for {symbol}. Skipping duplicate request.'
                )
                return
        ...
        try:
            ...
            self.logger.debug(
                f'Calculated duration: {duration_days} days for {points_to_request} bars @ {self.settings.TIMEFRAME}'
            )
        except Exception as e:
            self.logger.warning(
                f"Could not estimate duration from timeframe '{self.settings.TIMEFRAME}': {e}. Using default."
            )
        ...
        self.logger.info(
            f'Requesting historical data for {symbol} (ReqId: {req_id}), '
        ...
        try:
            ...
            self.api.reqHistoricalData(
                req_id, contract, '', duration_str, self.settings.TIMEFRAME, what_to_show, use_rth, 1, False, []
            )
        except Exception as e:
            self.error_logger.error(
                f'Exception during reqHistoricalData for {symbol} (ReqId: {req_id}): {e}', exc_info=True
            )
    ...
    def finalize_historical_data(self, symbol):
        ...
        if not buffered_bars:
            ...
            if not hist_df.empty:
                ...
                try:
                    ...
                    hist_df[['open', 'high', 'low', 'close']] = hist_df[
                        ['open', 'high', 'low', 'close']
                    ].ffill()  # Use ffill for OHLC
                    hist_df['volume'] = hist_df['volume'].fillna(0)
                    hist_df.dropna(
                        subset=['timestamp', 'open', 'high', 'low', 'close'], inplace=True
                    )  # Drop rows where OHLC still NaN
        ...
        with self._lock:  # Protect market_data access
            ...
            if live_df is not None and not live_df.empty:
                ...
                if not isinstance(live_df.index, pd.DatetimeIndex) or live_df.index.tz is None:
                    try:
                        ...
                        self.error_logger.error(
                            f'Failed to convert existing live data index to UTC for {symbol}: {e}. Discarding old live data.'
                        )
            ...
            if final_count > 0:
                self.save_market_data(symbol, data_type='historical')
        ...
        if symbol not in self.api.unsubscribed_symbols:
            ...
            self.logger.warning(
                f'Skipping live data subscription for {symbol} as it was previously marked unsubscribed.'
            )
    ...
    def get_exchange_tz(self, symbol):
        ...
        if details and isinstance(details, ContractDetails) and details.timeZoneId:
            try:
                ...
                self.logger.warning(
                    f"Unknown timezone '{details.timeZoneId}' from ContractDetails for {symbol}. Falling back."
                )
        ...
        try:
            ...
            self.logger.warning(
                f"Unknown timezone '{tz_name}' for exchange '{exchange}' in settings. Using default {DEFAULT_TZ_STR}."
            )
            return DEFAULT_TZ
    ...
    def is_market_open(self, symbol):
        ...
        datetime.now(pytz.utc)  # For comparison
        ...
        if sec_type == 'STK':
            ...
            try:
                ...
                ny_tz = DEFAULT_TZ  # Fallback
            now_ny_time = datetime.now(ny_tz)
            day_of_week = now_ny_time.weekday()
            current_ny_time = now_ny_time.time()
            is_weekend_closure = (
                (day_of_week == 4 and current_ny_time >= dt_time(17, 0))
                or day_of_week == 5
                or (day_of_week == 6 and current_ny_time < dt_time(17, 0))
            )
    ...
    def check_pause_conditions(self, symbol):
        ...
        if not is_open:
            if sec_type == 'STK':
                should_pause, pause_reason = True, 'Market Closed'
            elif sec_type == 'CASH':
                should_pause, pause_reason = True, 'Forex Market Closed (Weekend)'
    ...
    def _run_training_task(self):
        ...
        try:
            ...
            if self._retrain_future and self._retrain_future.done() and self._retrain_future.exception():
    ...
    def main_loop(self):
        ...
        while self.running:
            try:
                ...
                # --- Periodic Tasks ---
                if current_time_utc - self.last_state_save_time > timedelta(
                    seconds=self.settings.STATE_SAVE_INTERVAL_SECONDS
                ):
                    self.state_manager.save_state()
                    self.last_state_save_time = current_time_utc
                if current_time_utc - self.last_reconciliation_time > timedelta(
                    seconds=self.settings.RECONCILIATION_INTERVAL_SECONDS
                ):
                    if self.api.is_connected():
                        self.api.request_account_and_positions()
                        self.last_reconciliation_time = current_time_utc
                if current_time_utc - self.last_risk_check_time > timedelta(seconds=60):
                    ...
                    self.last_risk_check_time = current_time_utc
                ...
                for symbol in list(self.settings.TRADE_INSTRUMENTS):  # Iterate over copy
                    ...
                    if bar_queue:
                        try:
                                ...
                                if new_bar_df is not None and not new_bar_df.empty:
                                    with self._lock:  # Lock access to shared market_data
                                        ...
                                        if current_df is None or current_df.empty:
                                            ...
                                            if (
                                                not isinstance(current_df.index, pd.DatetimeIndex)
                                                or current_df.index.tz is None
                                            ):
                                                try:
                                                    current_df = current_df.set_index(
                                                        pd.to_datetime(current_df.index, utc=True)
                                                    )
                                                except Exception:
                                                    current_df = pd.DataFrame()  # Discard on error
                                    ...
                                    self.logger.debug(
                                        f'Processed new bar for {symbol}: {ts_str} C={new_bar["close"]:.5f}'
                                    )
                            ...
                            self.error_logger.error(f'Error processing bar queue for {symbol}: {e}', exc_info=True)
                ...
                if current_time_utc - self.last_stale_data_check_time > stale_check_interval:
                    ...
                    for symbol in self.settings.TRADE_INSTRUMENTS:
                        ...
                        if latest_price_info:
                            ...
                            if (current_time_utc - price_time_utc) > timedelta(
                                seconds=self.settings.MAX_DATA_STALENESS_SECONDS
                            ):
                                is_stale = True
                        ...
                        with self._lock:
                            ...
                            if is_stale and not currently_paused:
                                ...
                                not is_stale and currently_paused and 'Stale' in self.pause_reason.get(symbol, '')
                            ):  # Unpause if ONLY paused for staleness
                                # More complex logic needed if multiple pause reasons can exist
                                self.symbol_trading_paused[symbol] = False
                                self.logger.info(
                                    f'Market data for {symbol} is no longer stale. Resuming trading for symbol.'
                                )
                ...
                for symbol in self.settings.TRADE_INSTRUMENTS:
                    ...
                    if self.risk_manager.is_trading_halted():
                        if symbol == self.settings.TRADE_INSTRUMENTS[0]:  # Log once per cycle
                            self.logger.warning(
                                f'Skipping evaluations: Trading halted ({self.risk_manager.get_halt_reason()}).'
                            )
                        continue
                    ...
                    current_atr = None
                    if (
                        self.settings.ENABLE_ADAPTIVE_RISK
                        or 'ATR' in self.settings.STOP_LOSS_TYPE
                        or 'ATR' in self.settings.TAKE_PROFIT_TYPE
                    ):
                        if len(market_data_df) >= self.settings.ATR_PERIOD + 1:
                            try:
                                ...
                                if pd.notna(latest_atr_val) and latest_atr_val > self.settings.MIN_ATR_VALUE:
                                    ...
                                    with self._lock:
                                        current_atr = self.latest_atr.get(symbol)
                            except Exception as e:
                    ...
                    final_signal, individual_signals = self.strategy_manager.aggregate_signals(
                        symbol, market_data_df.copy()
                    )
                ...
                halt_status = (
                    f'HALTED ({self.risk_manager.get_halt_reason()})'
                    if self.risk_manager.is_trading_halted()
                    else 'Running'
                )
                ...
                status_line = (
                    f'\r[{now_local_str} {DEFAULT_TZ.zone}] Eq:{equity:,.2f} | '
        ...
        with self._lock:  # Protect market_data access
            ...
            if df is not None:
                ...
                if not isinstance(df_copy.index, pd.DatetimeIndex) or df_copy.index.tz is None:
                    try:
                        df_copy = df_copy.set_index(pd.to_datetime(df_copy.index, utc=True))
                    except Exception:
                        return None  # Return None if index correction fails
                return df_copy
            else:
                return None
    ...
    def attempt_trade_execution(self, symbol, signal, strategy_signals, current_atr):
        ...
        if is_closing_trade or is_reversing_trade:
            ...
            if quantity < min_trade_size:
                self.logger.warning(
                    f'Attempting to close small position ({quantity:.6f}) for {symbol}, less than min size {min_trade_size}. Skipping closure signal.'
                )
                return
            ...
            if self.settings.STOP_LOSS_TYPE == 'PERCENT':
                ...
                if current_atr is None or current_atr <= self.settings.MIN_ATR_VALUE:
                    self.logger.error(
                        f'Cannot size trade for {symbol}: Valid ATR ({current_atr}) needed for ATR Stop Loss.'
                    )
                    return
            ...
            if action == 'BUY' and stop_loss_price_calc >= latest_price:
                stop_loss_price_calc = round_price(latest_price - min_tick, min_tick)
            elif action == 'SELL' and stop_loss_price_calc <= latest_price:
                stop_loss_price_calc = round_price(latest_price + min_tick, min_tick)
            ...
            quantity = calculate_position_size(
                ...
                risk_amount=risk_capital_amount,
                available_funds=available_funds,
                symbol=symbol,
                min_trade_size=min_trade_size,
                min_tick=min_tick,
                api=self.api,
                commission_per_unit=estimated_commission,
                slippage_per_unit=estimated_slippage,
            )

            if quantity > 0:
                effective_sl_atr_mult_str = (
                    f'{base_sl_atr_multiplier * vol_multiplier:.2f}' if self.settings.STOP_LOSS_TYPE == 'ATR' else 'N/A'
                )
                self.logger.info(
                    f'Signal to open {action} position for {symbol}. Calc Qty: {quantity:.6f} (Vol: {vol_level}, SL ATR Mult: {effective_sl_atr_mult_str})'
                )
            else:
                self.logger.warning(
                    f'Calculated quantity is zero/negative for {symbol} ({quantity:.6f}). Skipping trade.'
                )
                return
        else:
            self.logger.debug(
                f'Ignoring signal for {symbol}: Not opening/closing/reversing based on current position ({current_position}).'
            )
            return

        if quantity < min_trade_size:
            self.logger.warning(
                f'Attempting {action} for {symbol} failed: Final quantity {quantity:.8f} is less than min trade size {min_trade_size:.8f}.'
            )
            return
        ...
        if quantity <= 0:
            self.logger.warning(
                f'Attempting {action} for {symbol} failed: Quantity became zero after rounding to min size increment {min_trade_size:.8f}.'
            )
            return
        ...
        if self.settings.STOP_LOSS_TYPE == 'PERCENT':
            ...
            if current_atr is None or current_atr <= self.settings.MIN_ATR_VALUE:
                self.error_logger.error(f'Cannot place trade {symbol}: Valid ATR needed for SL price ({current_atr}).')
                return
        ...
        if self.settings.TAKE_PROFIT_TYPE == 'PERCENT':
            ...
            if current_atr is None or current_atr <= self.settings.MIN_ATR_VALUE:
                self.error_logger.error(f'Cannot place trade {symbol}: Valid ATR needed for TP price ({current_atr}).')
                return
            ...
            self.logger.error('Invalid TAKE_PROFIT_TYPE.')
            return
        ...
        if action == 'BUY':
            ...
            if place_sl >= place_tp:
                self.logger.error(
                    f'Invalid SL/TP for BUY {symbol}: SL ({place_sl:.5f}) >= TP ({place_tp:.5f}). Cannot place trade.'
                )
                return
        else:  # SELL
            ...
            if place_sl <= place_tp:
                self.logger.error(
                    f'Invalid SL/TP for SELL {symbol}: SL ({place_sl:.5f}) <= TP ({place_tp:.5f}). Cannot place trade.'
                )
                return
        ...
        parent_id, stop_id, profit_id = self.order_manager.create_and_place_bracket_order(
            ...
            quantity=quantity,
            entry_price=latest_price,
            stop_loss_price=place_sl,
            take_profit_price=place_tp,
            strategy_name=strategy_name_log,
        )

        if parent_id:
            self.trade_logger.info(
                f'Successfully submitted bracket order for {symbol}. ParentID: {parent_id}, SL: {stop_id}, TP: {profit_id}'
            )
        else:
...
Examples:
  ...
  python main.py --headless --mode stock --instruments "AAPL,MSFT" --no-autonomous --extended-hours
    ...
    parser.add_argument(
        '--mode', type=str, choices=['stock', 'crypto', 'forex'], help='Trading mode (stock, crypto, forex)'
    )
    parser.add_argument('--instruments', type=str, help='Comma-separated list of instruments (e.g., "BTC/USD,ETH/USD")')
    parser.add_argument(
        '--autonomous',
        dest='autonomous',
        action='store_true',
    ...
    parser.add_argument('--no-autonomous', dest='autonomous', action='store_false', help='Disable autonomous mode')
    parser.add_argument(
        '--adaptive-risk',
        dest='adaptive_risk',
        action='store_true',
    ...
    parser.add_argument('--no-adaptive-risk', dest='adaptive_risk', action='store_false', help='Disable adaptive risk')
    parser.add_argument(
        '--auto-retrain',
        dest='auto_retrain',
        action='store_true',
    ...
    parser.add_argument(
        '--no-auto-retrain', dest='auto_retrain', action='store_false', help='Disable auto ML retraining'
    )
    parser.add_argument(
        '--dynamic-weighting',
        dest='dynamic_weighting',
        action='store_true',
        default=True,
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.headless:
            ...
            while bot and bot.running:
                ...
                if bot.api and bot.api.api_thread and not bot.api.api_thread.is_alive() and bot.api.is_connected():
                    ...
                    alive = False
                if (
                    hasattr(bot, 'data_aggregator')
                    and bot.data_aggregator.thread
                    and not bot.data_aggregator.thread.is_alive()
                    and bot.data_aggregator.running
                ):
        ...
        if bot and bot.running:
            bot.stop(reason='Keyboard Interrupt')
        elif bot and hasattr(bot, 'api') and bot.api.is_connected():
            bot.api.disconnect_app()
        print('Exiting.')
    except Exception as e:
        print(f'\nA critical unhandled error occurred: {e}. Check error logs. Forcing exit.')
        logger_to_use = (
            startup_error_logger
            if 'startup_error_logger' in locals()
            else setup_logger('main_critical', 'logs/error_logs/errors.log', level='CRITICAL')
        )
        logger_to_use.critical(f'Unhandled exception in main execution block: {e}', exc_info=True)
        if bot and bot.running:
            bot.stop(reason='Critical Error')
        elif bot and hasattr(bot, 'api') and bot.api.is_connected():
            bot.api.disconnect_app()
        sys.exit(1)
    finally:
