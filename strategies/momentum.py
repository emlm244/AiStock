...
class MomentumStrategy:
    def __init__(self):
        ...
        self.logger = setup_logger('Momentum', 'logs/strategies.log', level=self.settings.LOG_LEVEL)
        ...
        try:
            ...
            if not isinstance(self._initial_price_change_period, int) or self._initial_price_change_period <= 0:
                self.logger.warning(
                    f'Initial MOMENTUM_PRICE_CHANGE_PERIOD ({self._initial_price_change_period}) invalid. Using default 5 for min_data_points.'
                )
                self._initial_price_change_period = 5
        except AttributeError:
            self.logger.error(
                'MOMENTUM_PRICE_CHANGE_PERIOD not found in settings. Using default 5 for min_data_points.'
            )
            self._initial_price_change_period = 5
    ...
    def generate_signal(self, data):
        ...
        try:
            ...
            if latest_data[['price_change', 'volume', 'avg_volume']].isnull().any():
                self.logger.warning(
                    'NaN values encountered in latest data for Momentum check (PriceChg/Vol/AvgVol). No signal.'
                )
                return 0
