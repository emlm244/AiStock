...
class TrendFollowingStrategy:
    def __init__(self):
        ...
        self.logger = setup_logger('TrendFollowing', 'logs/strategies.log', level=self.settings.LOG_LEVEL)
        ...
        try:
            ...
            if self._initial_short_period >= self._initial_long_period:
                self.logger.warning(
                    f'Initial MA config invalid: short ({self._initial_short_period}) >= long ({self._initial_long_period}). Using defaults 9/21 for min_data_points.'
                )
                self._initial_short_period = 9
                self._initial_long_period = 21
        except (KeyError, TypeError) as e:
            self.logger.error(
                f'Could not read initial MA periods from settings: {e}. Using defaults 9/21 for min_data_points.'
            )
            self._initial_short_period = 9
            self._initial_long_period = 21
    ...
    def generate_signal(self, data):
        ...
        try:
            ...
            # --- Input Validation ---
            if (
                not isinstance(short_term_period, int)
                or not isinstance(long_term_period, int)
                or short_term_period <= 0
                or long_term_period <= 0
            ):
                self.logger.error(
                    f'Invalid MA periods in settings: short={short_term_period}, long={long_term_period}. No signal.'
                )
                return 0
            if short_term_period >= long_term_period:
                self.logger.error(
                    f'Short MA period ({short_term_period}) must be less than Long MA period ({long_term_period}). No signal.'
                )
                return 0
            ...
            if pd.isna(latest_short_ma) or pd.isna(latest_long_ma):
                self.logger.warning(
                    f'NaN value encountered in calculated moving averages (Short: {latest_short_ma}, Long: {latest_long_ma}). Check data quality. No signal.'
                )
                return 0

            # Log the latest moving average values for debugging
            self.logger.debug(
                f'Trend Check: Short MA({short_term_period})={latest_short_ma:.5f}, Long MA({long_term_period})={latest_long_ma:.5f}'
            )
