...
class IBKRApi(EWrapper, EClient):
    ...
    def connect_app(self):
        ...
        try:
            self.connect_thread = threading.Thread(
                target=self._execute_connect, args=(host, port, client_id), daemon=True
            )
        ...
        try:
            ...
            if not initial_data_ok:
                self.error_logger.critical(
                    'Failed to get critical initial data (OrderID/Account Summary) after connection. Bot cannot proceed safely.'
                )
                self.disconnect_app()
                if self.trading_bot:
                    self.trading_bot.stop('Initial Data Failed')
                return
            else:
    ...
    def _wait_for_initial_data(self, timeout=60):
        ...
        if self._next_valid_order_id is None:
            ...
            self.logger.info(f'NextValidId received: {self._next_valid_order_id}')
        ...
        if not summary_received:
            ...
            if 'NetLiquidation' not in self.account_details or 'TotalCashValue' not in self.account_details:
                self.error_logger.error(
                    'Essential account values (NetLiquidation/TotalCashValue) missing after timeout.'
                )
                critical_data_ok = False
        else:
            self.logger.info('Account Summary End received.')
        ...
        if not pos_received:
            self.logger.warning(f'Timeout ({pos_timeout:.1f}s) waiting for Positions End signal.')
        else:
            self.logger.info('Positions End received.')

        ord_timeout = max(0.1, timeout - (time.time() - start_time))
        ord_received = self.open_orders_received.wait(timeout=ord_timeout)
        if not ord_received:
            self.logger.warning(f'Timeout ({ord_timeout:.1f}s) waiting for Open Orders End signal.')
        else:
            self.logger.info('Open Orders End received.')
    ...
    def request_contract_details(self, symbol):
        ...
        with self.api_lock:
            ...
            try:
                ...
                self.error_logger.error(
                    f'Exception during reqContractDetails for {symbol} (ReqId: {req_id}): {e}', exc_info=True
                )
                if req_id in self.active_contract_detail_reqs:
                    del self.active_contract_detail_reqs[req_id]
                if symbol in self._pending_detail_reqs:
    ...
    def contractDetails(self, reqId: int, contractDetails: ContractDetails):
        ...
        if symbol:
            self.logger.info(
                f'Received Contract Details for: {symbol} (ReqId: {reqId}) - MinTick: {contractDetails.minTick}, Market Name: {contractDetails.marketName}'
            )
            with self.api_lock:
        ...
        with self._order_id_lock:
            ...
            while self._next_valid_order_id is None:
                ...
                if wait_time > max_wait:
                    self.error_logger.critical(
                        f'FATAL: Waited {max_wait}s but NextValidId not available. Cannot place orders.'
                    )
                    self.schedule_reconnect()
    ...
    def error(self, reqId: int, errorCode: int, errorString: str):
        ...
        symbol_context = 'N/A'
        if reqId in self.active_market_data_reqs:
            symbol_context = f'MktData:{self.active_market_data_reqs[reqId]}'
        elif reqId in self.active_hist_data_reqs:
            symbol_context = f'HistData:{self.active_hist_data_reqs[reqId]}'
        elif reqId in self.active_contract_detail_reqs:
            symbol_context = f'ContDet:{self.active_contract_detail_reqs[reqId]}'
        elif reqId == -1:
            symbol_context = 'System'
        else:
            if self.order_manager:
                order_data = self.order_manager.get_order_details(reqId)
                if order_data:
                    symbol_context = f'Order:{order_data["contract"].symbol}'
        ...
        if errorCode in connection_errors:
            ...
            symbol = None
            if reqId in self.active_market_data_reqs:
                symbol = self.active_market_data_reqs[reqId]
            elif reqId in self.active_hist_data_reqs:
                symbol = self.active_hist_data_reqs[reqId]
            ...
            if symbol:
                if errorCode in {10090, 10091, 10167, 354, 300}:
                    ...
                    with self.api_lock:
                        self.active_hist_data_reqs.pop(reqId, None)
    ...
    def position(self, account: str, contract: Contract, position: float, avgCost: float):
        ...
        if account == self.account_id:
            symbol = (
                f'{contract.symbol}/{contract.currency}' if contract.secType in ['CASH', 'CRYPTO'] else contract.symbol
            )
    ...
    def commissionReport(self, commissionReport: CommissionReport):
        ...
        self.logger.info(
            f'Commission Report - ExecID:{commissionReport.execId}, Comm:{commissionReport.commission} {commissionReport.currency}, RealizedPnL:{commissionReport.realizedPNL}'
        )
        try:
            if self.portfolio_manager:
    ...
    def tickPrice(self, reqId: int, tickType: int, price: float, attrib):
        ...
        if symbol:
            if symbol in self.unsubscribed_symbols or price <= 0:
                return
    ...
    def tickSize(self, reqId: int, tickType: int, size: int):
        ...
        if symbol:
            if symbol in self.unsubscribed_symbols or effective_size < 0:
                return
            tick_time_utc = datetime.now(pytz.utc)
    ...
    def subscribe_market_data(self, symbol, req_id=None):
        with self.api_lock:
            ...
            if existing_reqs:
                self.logger.debug(
                    f'{symbol} already subscribed with ReqId(s) {existing_reqs}. Skipping new subscription request {req_id}.'
                )
                return True
    ...
    def placeOrder(self, orderId, contract, order):
        ...
        try:
            ...
            time.sleep(0.05)
            self.logger.info(
                f'Placing Order -> ID:{orderId}, Symbol:{contract.symbol}, Action:{order.action}, Type:{order.orderType}, Qty:{order.totalQuantity}, LmtPx:{order.lmtPrice}, AuxPx:{order.auxPrice}, ParentId:{order.parentId}'
            )
            super().placeOrder(orderId, contract, order)
            return True
        except Exception as e:
            self.error_logger.error(
                f'Exception in placeOrder (ID: {orderId}, Symbol: {contract.symbol}): {e}', exc_info=True
            )
    ...
    def cancelMktData(self, reqId):
        ...
        if not self.isConnected():
            ...
            with self.api_lock:
                self.active_market_data_reqs.pop(reqId, None)
            return False
        try:
    ...
    def is_connected(self):
        ...
        try:
            client_connected = super().isConnected()
        except Exception:
            client_connected = False

        if not client_connected and self.connected:
            self.logger.warning(
                "Internal 'connected' flag was True, but EClient.isConnected() is False. Updating internal state."
            )
            self.connected = False
            ...
            self.logger.warning(
                "Internal 'connected' flag was False, but EClient.isConnected() is True. Correcting internal state."
            )
            self.connected = True
